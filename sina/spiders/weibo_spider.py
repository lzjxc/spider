...
class WeiboSpider(Spider):
    ...
    def parse_further_information(self, response):
        print("start parse further information")
        text = response.text
        ...
        yield Request(url=self.base_url + '/{}/fans?page=1'.format(information_item['_id']),
                      ...
                      dont_filter=True)
        print("end parse further information")
    ...
    def parse_tweet(self, response):
        ...
        for tweet_node in tweet_nodes:
            try:
                ...
                self.logger.error(e)
        print("end parse tweet ")


    def parse_all_content(self, response):
        print("start parse all content")
        # 有阅读全文的情况，获取全文
        tree_node = etree.HTML(response.body)
    ...
    def parse_follow(self, response):
        ...
        print("END parse follow")
    def parse_fans(self, response):
        """
        抓取粉丝列表
        """
        # 如果是第1页，一次性获取后面的所有页
        print("start parse FANS")
        if response.url.endswith('page=1'):
            all_page = re.search(r'/>&nbsp;1/(\d+)页</div>', response.text)
            if all_page:
                all_page = all_page.group(1)
        ...
        for uid in uids:
            ...
            yield relationships_item
        print("END parse FANS")

    def parse_comment(self, response):
        print("start parse comment")
        # 如果是第1页，一次性获取后面的所有页
        if response.url.endswith('page=1'):
            all_page = re.search(r'/>&nbsp;1/(\d+)页</div>', response.text)
            if all_page:
...
if __name__ == "__main__":
    ...
    process.start()
    #timer = threading.Timer(5, addNewIp(self))
    #timer.start()
